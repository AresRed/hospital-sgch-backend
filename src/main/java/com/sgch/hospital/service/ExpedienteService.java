...
public class ExpedienteService {
        ...
        @Transactional
        public NotaMedica finalizarYGuardarExpediente(Long citaId, Doctor doctor, FinalizarCitaRequest request)
                        throws Exception {
        ...
        public Expediente obtenerExpedientePorPacienteId(Long pacienteId) throws Exception {
                ...
                // Forzar la inicialización de la colección de notas dentro de la transacción
                expediente.getNotas().size();
        ...
        public Receta obtenerUltimaReceta(Long pacienteId) throws Exception {
                ...
                return expediente.getNotas().stream()
                                .sorted((n1, n2) -> n2.getFechaHora().compareTo(n1.getFechaHora())) // Ordenar por fecha
                                                                                                    // descendente
        ...
        public Expediente actualizarDatosFijos(Long pacienteId, ExpedienteUpdateDTO dto) throws Exception {

                Paciente paciente = pacienteRepository.findById(pacienteId)
                                .orElseThrow(() -> new Exception("Paciente no encontrado."));

                // Obtener el expediente o crearlo si no existe (aunque debería existir si ya ha
                // sido atendido)
                Expediente expediente = expedienteRepository.findByPaciente(paciente)
                                .orElseGet(() -> {
                ...
                // Aplicar los cambios
                if (dto.getHistorialAlergias() != null) {
                        expediente.setHistorialAlergias(dto.getHistorialAlergias());
                }
                if (dto.getGrupoSanguineo() != null) {
                        expediente.setGrupoSanguineo(dto.getGrupoSanguineo());
                }
                if (dto.getObservacionesGenerales() != null) {
                        expediente.setObservacionesGenerales(dto.getObservacionesGenerales());
                }
